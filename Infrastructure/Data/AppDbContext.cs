...
namespace Infrastructure.Data
{
    ...
    {
        public DbSet<ValueRecord> ValueRecords { get; set; } //  аждое DbSet<T>-таблица в Ѕƒ
        public DbSet<ResultRecord> ResultRecords { get; set; }

        public AppDbContext(DbContextOptions<AppDbContext> options) : base(options) { }
        // ѕередает настройки в базовый класс EF Core
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<ValueRecord>(entity =>
            {
                entity.HasKey(v => v.Id);
                entity.Property(v => v.Date).IsRequired(); // IsRequired()=поле не может быть NULL в Ѕƒ
                entity.Property(v => v.ExecutionTime).IsRequired().HasPrecision(18, 6); // рем€ выполнени€ операции
            ...
            modelBuilder.Entity<ResultRecord>(entity =>
            {
                ...
                entity.Property(r => r.DeltaTimeSeconds).IsRequired().HasPrecision(18, 6); //  разница во времени между двум€ моментами
                ...
                // медианное значение набора данных (статистическа€ мера центральной тенденции)
                ...
                // временна€ метка, котора€ показывает когда запись была создана/обработана
